...
class Client extends EventEmitter {
    constructor(options) {
		...
		// Try to fix client disconnect but cannot register the same id
		// Date: 16 Jan 2024
		agent.once('end', (err) => {
			this.debug('client end %s', id);
    ...
    handleRequest(req, res) {
        ...
        const clientReq = http.request(opt, (clientRes) => {
            this.debug('response %s < %s', clientRes.statusCode, req.url);

...
class TunnelAgent extends Agent {
    constructor(options = {}) {
		...
		var key = `localtunnel:TunnelAgent[${options.clientId}]`;
	...
	/**
	 * @desc Listen a tcp server at random port. That port will using to client make connection to server
	 * @returns {number|string} port
	 */
    listen() {
